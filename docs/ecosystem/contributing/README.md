...
If you are not using `update-release-info`, you will need to determine the package you are changing uses the continuous or [batched] release model (https://github.com/flutter/flutter/blob/master/docs/ecosystem/release/README.md#batch-release), and follow the approriate instructions below. If the package has a top-level `ci_config.yaml` setting `release:batch:true`, then it uses batched release, otherwise it defaults to continuous release.
